...
def setup_maze(grid):                          # define a function called setup_maze
    ...
    for y in range(len(grid)):                 # read in the grid line by line
        for x in range(len(grid[y])):          # read each cell in the line
            character = grid[y][x]             # assign the variable "character" the the x and y location od the grid
            ...
            if character == "e":
                ...
                green.stamp()
                green.color("green") # color the end location to green

            if character == "s":
                start_x, start_y = screen_x, screen_y  # assign start locations variables to start_x and start_y
                red.goto(screen_x, screen_y) # color the start location to red
...
def search(x,y): # Breadth First Search Algorithm
    frontier.append((x, y))
    solution[x, y] = x, y
    ...
    while len(frontier) != 0:          # exit while loop when frontier queue equals zero
        ...
        # checking for all four directions: up, down, left, right
...
def backRoute(x, y):
    ...
    while (x, y) != (start_x, start_y):    # stop loop when current cells == start cell
        turquoise.goto(solution[x, y])        # move the yellow sprite to the key value of solution ()
        turquoise.stamp()
